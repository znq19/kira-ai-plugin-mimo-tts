...
AI 会自动生成 `<mimo_tts>` 标签触发语音合成，把语音发出。
...
描述里可以带上**性别、年龄、嗓音质感、语速、情感风格**，通常越具体效果越好。
...
如想更好指导AI调用，可在人设中写一句“语音原则：只（或写成“优先”）使用mimo tts插件，并根据语境和自己心情主动调用。”
...
会自动降级为文字发送，消息不会丢。检查 API 密钥和账户余额。（目前纯免费，以后不知道）
</details>

<details>
<summary>和官方 <code>&lt;record&gt;</code> 有什么区别？</summary>
<code>&lt;record&gt;</code> 使用webui内配置的语音合成提供商（但mimo tts不在此列）；<code>&lt;mimo_tts&gt;</code> 可自然语言自定义音色。两者互不冲突。
</details>

<details>
<summary>支持英文吗？</summary>
支持多种语言文本合成。音色描述建议用中文写。
</details>
